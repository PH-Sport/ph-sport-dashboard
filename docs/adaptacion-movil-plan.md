...
4. **Círculo «+» de crear separado a la derecha de la tab bar** (patrón iOS 26 Liquid Glass: píldora + círculo trailing, como Apple Music/Fitness; propuesto por Mario). Relleno `bg-primary` (el CTA de la app), abre `CreateDesignDialog` desde CUALQUIER sección y revalida las listas vía SWR global (`/api/designs*` + `['team-data',…]`). Los botones "Crear" del header pasan a ser solo de escritorio (`hidden md:inline-flex`) — en Inicio móvil se recupera esa fila. El diálogo se monta perezosamente al primer uso.
