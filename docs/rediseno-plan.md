...
- ✅ **PORT-6 Semana/Ajustes + limpieza** (2026-06-16): **Semana** repintada a placas D — `/equipo` (Mánager): placa por diseñador con lista inline (UrgencyDot, sobrecarga >5, header→`/equipo/[id]`, diseño→detalle centrado); `/mi-semana` (Diseñador): Pendientes con "Entregar" en un toque + Entregadas plegables por semana con vuelta atrás confirmada. Primitivo `WeekNav` real creado. **Ajustes**: scroll único (Cuenta·Apariencia·Notificaciones en placas, reusa hooks reales) + pestaña **Miembros** (solo Mánager) con tarjetas→popup; `/ajustes/usuarios` ahora redirige a `/ajustes?tab=miembros` (enlaces actualizados). **Miembros — acciones** (decisión usuario): renombrar (visible) + cambiar rol y eliminar en "Zona avanzada" plegada (extraordinarias, con confirmación). Backend: ruta `PATCH /api/users/[id]` (renombrar/rol, RLS ya autoriza admin, guardas: no auto-rol, no dejar sin Mánager) + edge function `admin-delete-user` (service-role en Supabase; reasigna `created_by` y borra cuenta auth → cascada conserva diseños sin asignar). **`/concepts` borrada.** `DesignCard`/`DeliveredSection` muertos eliminados. Build verde 16/16.
  - ⏳ **Pendiente PORT-6**: el **despliegue** de la edge function `admin-delete-user` fue bloqueado (acción destructiva en prod, decisión del usuario). El borrado de usuarios no funciona hasta desplegar: `supabase functions deploy admin-delete-user`. Renombrar/cambiar rol sí funcionan (ruta Next, viven en el deploy de la app). Validación visual del usuario pendiente.
  - **Decidido fase aparte**: cuenta developer (is_dev + conmutador de rol).
