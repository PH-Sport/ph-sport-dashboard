...
- **Server-side** (leen columna autoritativa, sin re-derivar):
  - Triggers de notificación (`004`, `009`): `SELECT full_name INTO
    commenter_name` → `SELECT display_name` (la notificación usa el mismo nombre
    corto que la UI).
  - Edge function de email (`send-notification-email`): selecciona y saluda con
    `display_name`; se elimina el `getGreetingName(...)` que extraía el primer
    nombre a mano.
...
- **Migración SQL nueva**: columnas + generadas + backfill + overrides +
  `handle_new_user` + funciones de notificación `004`/`009`.
...
6. **Server-side display**: funciones de notificación a `display_name`; edge
   function de email a `display_name` (deploy aparte).
